...
class BackgroundEstimatorProcessor(processor.ProcessorABC):
    ...
    def process(self, events: ak.Array):
        ...
        self.add_selection("one_lepton", ((n_good_muons == 1) & (n_good_electrons == 0)), ["mu"])
        self.add_selection("one_lepton", ((n_good_muons == 0) & (n_good_electrons == 1)), ["ele"])
        ...
        for ch in self._channels:  
            if not os.path.exists(self._output_location + date + "/" + ch):
                os.makedirs(self._output_location + date + "/" + ch)
            if not os.path.exists(self._output_location + date + "/" + ch + "/" + sample):
                os.makedirs(self._output_location + date + "/" + ch + "/" + sample)

            fname = self._output_location + date + "/" + ch + "/" + sample + "/" + partition_key
